...
describe FredApiController, :type => :controller do
  ...
  describe "GET facility" do
    ...
    it 'should get default fields' do
      get :show_facility, params: { id: site.id, format: 'json', collection_id: collection.id }
    ...
    it 'should get extended properties' do
      get :show_facility, params: { id: site_with_properties.id, format: 'json', collection_id: collection.id }
    ...
    it "should convert time in different timezone to UTC" do
      ...
      get :show_facility, params: { id: site2.id, format: 'json', collection_id: collection.id }
      json = JSON.parse response.body
  ...
  describe "query list of facilities" do
    ...
    it 'should get the full list of facilities' do
      get :facilities, params: { format: 'json', collection_id: collection.id }
    ...
    it 'should sort the list of facilities by name asc' do
      get :facilities, params: { format: 'json', sortAsc: 'name', collection_id: collection.id }
    ...
    it 'should sort the list of facilities by name desc' do
      get :facilities, params: { format: 'json', sortDesc: 'name', collection_id: collection.id }
    ...
    it 'should sort the list of facilities by property date' do
      get :facilities, params: { format: 'json', sortDesc: 'inagurationDay', collection_id: collection.id }
    ...
    describe 'limit' do
      ...
      it 'should limit the number of facilities returned and the offset for the query' do
        get :facilities, params: { format: 'json', limit: 1, collection_id: collection.id }
        json = (JSON.parse response.body)["facilities"]
        expect(json.length).to eq(1)
        expect(json[0]["name"]).to eq(site1.name)
        get :facilities, params: { format: 'json', limit: 1, offset: 1, collection_id: collection.id }
        json = (JSON.parse response.body)["facilities"]
        expect(json.length).to eq(1)
        expect(json[0]["name"]).to eq(site2.name)
      end
      ...
      it 'should not limit the number of facilities when limit=off' do
        get :facilities, params: { format: 'json', limit: "off", collection_id: collection.id }
        json = (JSON.parse response.body)["facilities"]
    ...
    it 'should select only default fields' do
      get :facilities, params: { format: 'json', fields: "name,updatedAt", collection_id: collection.id }
      json = (JSON.parse response.body)["facilities"]
      expect(json.length).to eq(2)
    ...
    it 'should select default and custom fields' do
      get :facilities, params: { format: 'json', fields: "name,properties:inagurationDay", collection_id: collection.id }
      json = (JSON.parse response.body)["facilities"]
      expect(json.length).to eq(2)
      expect(json[0].length).to eq(2)
      expect(json[0]['name']).to eq(site1.name)
    ...
    it 'should return all fields (default and custom) when parameter allProperties is set' do
      get :facilities, params: { format: 'json', allProperties: true, collection_id: collection.id }
      json = (JSON.parse response.body)["facilities"]
      expect(json.length).to eq(2)
    ...
    it "should select uuid field in partial response" do
      get :facilities, params: { format: 'json', fields: "uuid,properties:inagurationDay", collection_id: collection.id }
      json = (JSON.parse response.body)["facilities"]
      expect(json.length).to eq(2)
      expect(json[0].length).to eq(2)
  ...
  describe "Filtering Facilities" do
    ...
    it "should filter by name" do
      get :facilities, params: { format: 'json', name: site1.name, collection_id: collection.id }
      json = (JSON.parse response.body)["facilities"]
      expect(json.length).to eq(1)
      expect(json[0]['name']).to eq(site1.name)
    end
    ...
    it "should filter by coordinates" do
      get :facilities, params: { format: 'json', coordinates: [site1.lng.to_f, site1.lat.to_f], collection_id: collection.id }
      json = (JSON.parse response.body)["facilities"]
      expect(json.length).to eq(1)
      expect(json[0]['uuid']).to eq(site1.uuid)
    end
    ...
    it "should filter by updated_at" do
      ...
      get :facilities, params: { format: 'json', updatedAt: iso_updated_at, collection_id: collection.id }
      json = (JSON.parse response.body)["facilities"]
      expect(json.length).to eq(1)
    ...
    it "should filter by created_at" do
      ...
      get :facilities, params: { format: 'json', createdAt: iso_created_at, collection_id: collection.id }
      json = (JSON.parse response.body)["facilities"]
      expect(json.length).to eq(1)
      expect(json[0]['uuid']).to eq(site3.uuid)
    end
    ...
    it "should filter by active" do
      ...
      get :facilities, params: { format: 'json', active: 'false', collection_id: collection.id }
      json = (JSON.parse response.body)["facilities"]
    ...
    it "should filter by updated since" do
      ...
      site1.save!
      get :facilities, params: { format: 'json', updatedSince: iso_before_update, collection_id: collection.id }
      json = (JSON.parse response.body)["facilities"]
      expect(json.length).to eq(1)
      expect(json[0]['uuid']).to eq(site1.uuid)
    end
    ...
    it "should filter by updated since with arbitrary updated_at velues" do
      ...
      get :facilities, params: { format: 'json', updatedSince: "2013-02-04T22:55:53Z", collection_id: collection.id }
      json = (JSON.parse response.body)["facilities"]
      expect(json.length).to eq(2)
    ...
    it "should filter by property with 'properties.' prefix" do
      get :facilities, params: { format: 'json', "properties.numBeds" => 55, collection_id: collection.id }
      json = (JSON.parse response.body)["facilities"]
      expect(json.length).to eq(1)
      expect(json[0]['name']).to eq(site1.name)
    end
    ...
    it "should filter by uuid" do
      get :facilities, params: { format: 'json', uuid: site1.uuid, collection_id: collection.id }
      json = (JSON.parse response.body)["facilities"]
      expect(json.length).to eq(1)
      expect(json[0]['uuid']).to eq(site1.uuid)
    end
  ...
  describe "delete facility" do
    it "should render json's code field 200 when deleting a facility" do
      ...
      delete :delete_facility, params: { id: site.id, collection_id: collection.id }
      json = JSON.parse response.body
    ...
    it "should render json's code field 404 when the site is not found in an empty collection" do
      ...
      delete :delete_facility, params: { id: 100, collection_id: collection.id }
      json = JSON.parse response.body
  ...
  describe "http status codes" do
    ...
    it "should return 403 if user is do not have permission to access the collection" do
      ...
      get :show_facility, params: { id: site.id, format: 'json', collection_id: collection2.id }
      expect(response.status).to eq(403)
    end
    ...
    it "should return 404 if the requested site does not exist" do
      get :show_facility, params: { id: 12355259, format: 'json', collection_id: collection.id }
    ...
    it "should return 400 if a non existing field is included in the query" do
      get :facilities, params: { format: 'json', invalid: "option", collection_id: collection.id }
  ...
  describe "should update facility" do
    ...
    it "should return 404 if the facility does not exist" do
      put :update_facility, params: { collection_id: collection.id, id: "124566", :name => "Kakamega HC 2" }
      expect(response.status).to eq(404)
    end

    it "should update name" do
      request.env["RAW_POST_DATA"] = { :name => "Kakamega HC 2" }.to_json
      put :update_facility, params: { collection_id: collection.id, id: site.id }
    ...
    describe "partial updates" do
      ...
      it "should update a single property" do
        ...
        request.env["RAW_POST_DATA"] = {properties: json_data}.to_json
        put :update_facility, params: { collection_id: collection.id, id: site_with_properties.id }
  ...
  describe "Should create facility" do
    it "should not create a facility without a name" do
      post :create_facility, params: { collection_id: collection.id }
      expect(response.status).to eq(400)
      json = JSON.parse response.body
  ...
  describe "External Facility Identifiers" do
    ...
    it "should return identifiers in single facility query" do
      get :show_facility, params: { id: site_with_metadata.id, format: 'json', collection_id: collection.id }
      json = JSON.parse response.body
    ...
    it 'should filter by identifier', focus: true do
      get :facilities, params: { format: 'json',  collection_id: collection.id, "identifiers.id" => "53adf" }
      json = (JSON.parse response.body)["facilities"]
      expect(json.length).to eq(1)
    ...
    it 'should filter by identifier and agency' do
      get :facilities, params: { format: 'json',  collection_id: collection.id, "identifiers.agency" => "DHIS", "identifiers.id" => "53adf" }
      json = (JSON.parse response.body)["facilities"]
      expect(json.length).to eq(1)
      expect(json[0]['uuid']).to eq("#{site_with_metadata.uuid}")
    end
    ...
    it 'should filter by identifier and context' do
      get :facilities, params: { format: 'json',  collection_id: collection.id, "identifiers.context" => "MOH", "identifiers.id" => "53adf" }
      json = (JSON.parse response.body)["facilities"]
      expect(json.length).to eq(1)
      expect(json[0]['uuid']).to eq("#{site_with_metadata.uuid}")
    end
    ...
    it 'should filter by identifier, context and agency' do
      get :facilities, params: { format: 'json',  collection_id: collection.id, "identifiers.context" => "MOH", "identifiers.id" => "53adf", "identifiers.agency" => "DHIS" }
      json = (JSON.parse response.body)["facilities"]
      expect(json.length).to eq(1)
      expect(json[0]['uuid']).to eq("#{site_with_metadata.uuid}")
    end

    it 'sholud return an empty list if the id does not match' do get :facilities, params: { format: 'json',  collection_id: collection.id, "identifiers.context" => "MOH", "identifiers.id" => "invalid", "identifiers.agency" => "DHIS" }
      json = (JSON.parse response.body)["facilities"]
      expect(json.length).to eq(0)
    end

    it 'sholud return an empty list if the context does not match any identifier' do
      get :facilities, params: { format: 'json',  collection_id: collection.id, "identifiers.context" => "invalid", "identifiers.id" => "53adf", "identifiers.agency" => "DHIS" }
      json = (JSON.parse response.body)["facilities"]
      expect(json.length).to eq(0)
    end

    it 'sholud return an empty list if the agency does not match any identifier' do
      get :facilities, params: { format: 'json',  collection_id: collection.id, "identifiers.context" => "MOH", "identifiers.id" => "53adf", "identifiers.agency" => "invalid" }
      json = (JSON.parse response.body)["facilities"]
      expect(json.length).to eq(0)
    end
