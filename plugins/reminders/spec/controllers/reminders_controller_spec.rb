...
describe RemindersController, :type => :controller do
  ...
  it "should create reminder" do
    expect {
      post :create, params: { collection_id: collection.id, reminder: { name: "foo", reminder_date: "2012-05-06T00:30:00Z 0:00", reminder_message: "foo", repeat_id: repeat.id, collection_id: 1, sites: [site.id] } }
    }.to change { Reminder.count }.by 1
  end

  it "should update reminder" do
    put :update, params: { :id => reminder.id, :collection_id => collection.id, :reminder => { name: "foo" } }
    expect(reminder.reload.name).to eq("foo")
  end

  it "should destroy reminder" do
    expect {
      delete :destroy, params: { :id => reminder.id, :collection_id => collection.id }
    }.to change { Reminder.count }.by -1
  end

  it 'should update status' do
    post :set_status, params: { :id => reminder.id, :collection_id => collection.id, :status => true }
