describe 'ImportWizard', ->
  ...
  describe 'ValidationErrors', ->
    beforeEach ->
      ...
      columns =[{header: "field 1", use_as: "name"}, {header: "field 2", use_as: "field1"}]
    ...
    it 'should evaluate if there are errors in @errors', ->
      ...
      empty_val_errors = new ValidationErrors(window.model.columns, errors)
      expect(empty_val_errors.hasErrors()).toBe(false)
      errors.duplicated_code = {'text_column':[0, 1]}
      val_errors = new ValidationErrors(window.model.columns, errors)
    ...
    check_duplicated_field_assertion = (error_type, column_name, proc) ->
      ...
      proc(errors)
      val_errors = new ValidationErrors(window.model.columns, errors)
      readable_errors = val_errors.processErrors()
      expect(readable_errors.length).toBe(1)
      first_error = readable_errors[0]
      if error_type == 'name'
      ...
      expect(first_error.more_info).toEqual("Field 'field 1' (1st column) and field 'field 2' (2nd column) have the same #{error_type}. To fix this issue, leave only one with that #{error_type} and modify the rest.")
    ...
    check_existing_field_assertion = (error_type, column_name, proc) ->
      ...
      expect(first_error.columns).toEqual([0,1])
      expect(first_error.more_info).toEqual("Field 'field 1' (1st column) and field 'field 2' (2nd column) have #{error_type} #{column_name}. To fix this issue, change all their #{error_type}s.")
    ...
    check_duplicated_usage = (error_type, column_name, proc) ->
      ...
      expect(first_error.columns).toEqual([0,1])
      expect(first_error.more_info).toEqual("Field 'field 1' (1st column) and field 'field 2' (2nd column) are marked as #{column_name}. To fix this issue, leave only one of them assigned as '#{column_name}' and modify the rest.")
    ...
    check_missing_element_assertion_plural = (missing_element, proc) ->
      ...
      expect(first_error.description).toBe("Field 'field 2' (2nd column), the 3rd column and the 4th column are missing the field's #{missing_element}.")
      expect(first_error.columns).toEqual([1,2,3])
      expect(first_error.more_info).toEqual("Field 'field 2' (2nd column), the 3rd column and the 4th column are missing the field's #{missing_element}, which is required for new fields. To fix this issue, add a #{missing_element} for each of these columns.")
    ...
    check_missing_element_assertion = (missing_element, proc) ->
      ...
      if missing_element == 'label'
        missing_element = 'name'
      expect(first_error.description).toBe("Field 'field 2' (2nd column) is missing the field's #{missing_element}.")
      expect(first_error.columns).toEqual([1])
      expect(first_error.more_info).toEqual("Field 'field 2' (2nd column) is missing the field's #{missing_element}, which is required for new fields. To fix this issue, add a #{missing_element} for this column.")

    it "should generate readable errors for duplicated code", ->
    ...
    it "should generate readable errors for duplicated label", ->
    ...
    it "should generate readable errors for existing code", ->
    ...
    it "should generate readable errors for existing label", ->
    ...
    it "should generate readable errors for duplicated usage for default usages (lat, lng, name or id)", ->
    ...
    it "should generate readable errors for duplicated usage for existing_field", ->
    ...
    it "should generate readable errors for hierarchy field found", ->
      ...
      expect(first_error.columns).toEqual([1,2,3])
      expect(first_error.more_info).toEqual("Field 'field 2' (2nd column), the 3rd column and the 4th column.")

    it "should generate readable errors for more than one label missing for new fields", ->
    ...
    it "should generate readable errors for more than one code missing for new fields", ->
    ...
    it "should generate readable errors one label missing for new fields", ->
   ...
   it 'should generate readable errors for reserved code', ->
      ...
      expect(error.more_info).toEqual("Field 'field 1' (1st column) has code 'name'. To fix this issue, change its code.")

    it "should generate readable errors data errors", ->
      errors = {data_errors:[]}
      errors.data_errors = [{description: "Some options in column 5 don't exist.", column: 4, rows: [1,2], example: "", type: "options"}, {description: "Some of the values in the 2nd column are not valid for the type numeric.", column: 1, rows: [1], type: 'numeric values', example: "Values must be integers."}]
      val_errors = new ValidationErrors(window.model.columns, errors)
      readable_errors = val_errors.processErrors()
      expect(readable_errors.length).toBe(2)
      first_error = readable_errors[0]
      expect(first_error.error_kind).toBe("data_errors")
      expect(first_error.description).toBe("There are 2 invalid values in the 5th column.")
      ...
      second_error = readable_errors[1]
      expect(second_error.error_kind).toEqual("data_errors")
      expect(second_error.description).toEqual("There is 1 invalid value in field 'field 2' (2nd column).")
      expect(second_error.columns).toEqual([1])
      expect(second_error.more_info).toEqual("Some of the values in the 2nd column are not valid for the type numeric. To fix this, either change the column's type or edit your CSV so that all rows hold valid numeric values. Values must be integers. The invalid numeric values are in the following rows: 2.")
    ...
    it 'should generate messages in singular in existing_code when only one column has issues', ->
      ...
      expect(first_error.more_info).toEqual("Field 'field 1' (1st column) has code text_column. To fix this issue, change its code.")
    ...
    it 'should generate messages in singular in existing_label when only one column has issues', ->
      ...
      first_error = readable_errors[0]
      expect(first_error.columns).toEqual([0])
      expect(first_error.more_info).toEqual("Field 'field 1' (1st column) has name text_column. To fix this issue, change its name.")

    it 'should filter errors for a column', ->
      errors = {duplicated_code:[], duplicated_label:[], existing_label:[], existing_code:[], duplicated_usage: [], data_errors: []}
      errors.existing_code = {text_column: [0,1]}
      errors.duplicated_label = {other_column: [1, 2]}
      errors.data_errors = [{description: "Some options in column 5 don't exist.", column: 0, rows: [1,2], example: "", type: "options"}, {description: "Some of the values in the 2nd column are not valid for the type numeric.", column: 1, rows: [1], type: 'numeric values', example: "Values must be integers."}]
      errors.duplicated_code = {text_column: [0, 1]}
      val_errors = new ValidationErrors(window.model.columns, errors)
