...
describe Api::MembershipsController, :type => :controller do
  ...
  describe 'as admin' do
    ...
    it "should return all users not in collection as invitable" do
      new_user = User.make
      get :invitable, params: { collection_id: collection.id }
      json = JSON.parse response.body
    ...
    context 'create' do
      ...
      it "should create membership for new user" do
        new_user = User.make
        post :create, params: { collection_id: collection.id, email: new_user.email }
        json = JSON.parse response.body
      ...
      it "should return error for non-existant user" do
        post :create, params: { collection_id: collection.id, email: 'random@example.com' }
      ...
      it "should return error for non-existant collection" do
        new_user = User.make
        post :create, params: { collection_id: 0, email: new_user.email }
        expect(response.status).to eq(422)
      end

      it "should return the membership if it already exists" do
        post :create, params: { collection_id: collection.id, email: user.email }
    ...
    it "should delete a membership" do
      expect(collection.memberships.count).to eq(2)
      delete :destroy, params: { collection_id: collection.id, id: non_admin_user.id }
  ...
  describe 'as member' do
    ...
    it "should not delete a membership" do
      delete :destroy, params: { collection_id: collection.id, id: user.id }
  ...
  describe "admin flag" do
    ...
    it "should set admin" do
      post :set_admin, params: { collection_id: collection.id, id: non_admin_user.id }
    ...
    it "should unset admin" do
      ...
      expect(membership.admin).to be_truthy
      post :unset_admin, params: { collection_id: collection.id, id: non_admin_user.id }
      membership = collection.memberships.find_by_user_id non_admin_user.id
