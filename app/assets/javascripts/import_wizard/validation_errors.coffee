onImportWizard ->
  class @ValidationErrors
    ...
    columnsDescription: (index_array) =>
      index_array = $.map index_array, (index) => @columnDescription(index)
    ...
    columnDescription: (index) =>
      ...
      if $.trim(column?.header()).length > 0
        "field '#{column.header()}' (#{@ordinalize index + 1} column)"
      else
        "the #{@ordinalize index + 1} column"
          ...
          for errorId, errorColumns of errors
            ...
            switch errorType
              ...
              when 'duplicated_code'
                ...
                error_description.more_info = @capitalize "#{@columnsDescription(errorColumns)} have the same code. To fix this issue, leave only one with that code and modify the rest."
              when 'duplicated_label'
                error_description.description = "There is more than one column with name '#{errorId}'."
                error_description.more_info = @capitalize "#{@columnsDescription(errorColumns)} have the same name. To fix this issue, leave only one with that name and modify the rest."
              when 'missing_code'
                if errorColumns.length >1
                  error_description.description = @capitalize "#{@columnsDescription(errorColumns)} are missing the field's code."
                  error_description.more_info = @capitalize "#{@columnsDescription(errorColumns)} are missing the field's code, which is required for new fields. To fix this issue, add a code for each of these columns."
                else
                  error_description.description = @capitalize "#{@columnsDescription(errorColumns)} is missing the field's code."
                  error_description.more_info = @capitalize "#{@columnsDescription(errorColumns)} is missing the field's code, which is required for new fields. To fix this issue, add a code for this column."
              when 'missing_label'
                if errorColumns.length >1
                  error_description.description = @capitalize "#{@columnsDescription(errorColumns)} are missing the field's name."
                  error_description.more_info = @capitalize "#{@columnsDescription(errorColumns)} are missing the field's name, which is required for new fields. To fix this issue, add a name for each of these columns."
                else
                  error_description.description = @capitalize "#{@columnsDescription(errorColumns)} is missing the field's name."
                  error_description.more_info = @capitalize "#{@columnsDescription(errorColumns)} is missing the field's name, which is required for new fields. To fix this issue, add a name for this column."
              when 'duplicated_usage'
                ...
                error_description.more_info = @capitalize "#{@columnsDescription(errorColumns)} are marked as #{duplicated}. To fix this issue, leave only one of them assigned as '#{duplicated}' and modify the rest."
              when 'existing_code'
                error_description.description = "There is already a field with code #{errorId} in this collection."
                if errorColumns.length > 1
                  error_description.more_info = @capitalize "#{@columnsDescription(errorColumns)} have code #{errorId}. To fix this issue, change all their codes."
                else
                  error_description.more_info = @capitalize "#{@columnDescription(errorColumns[0])} has code #{errorId}. To fix this issue, change its code."
              when 'existing_label'
                error_description.description = "There is already a field with name #{errorId} in this collection."
                if errorColumns.length > 1
                  error_description.more_info = @capitalize "#{@columnsDescription(errorColumns)} have name #{errorId}. To fix this issue, change all their names."
                else
                  error_description.more_info = @capitalize "#{@columnDescription(errorColumns[0])} has name #{errorId}. To fix this issue, change its name."
              when 'hierarchy_field_found'
                error_description.description = "Hierarchy fields can only be created via web in the Layers page."
                error_description.more_info = @capitalize "#{@columnsDescription(errorColumns)}."
              when 'reserved_code'
              ...
              when 'non_existent_site_id'
                ...
                if error.rows.length == 1
                  error_description.description = "There is 1 issue with the value in #{@columnDescription(error.column)}."
                else
                  error_description.description = "There are #{error.rows.length} issues with the values in #{@columnDescription(error.column)}."
              ...
              when 'data_errors'
                ...
                error_description.columns = [error.column]
                if error.rows.length > 1
                  error_description.description = "There are #{error.rows.length} invalid values in #{@columnDescription(error.column)}."
                else
                  error_description.description = "There is #{error.rows.length} invalid value in #{@columnDescription(error.column)}."
