class LayersController < ApplicationController
  ...
  def index
    respond_to do |format|
      ...
      if current_snapshot
        format.json { render json: layers.includes(:field_histories).where("field_histories.valid_since <= '#{current_snapshot.date}' && ('#{current_snapshot.date}' < field_histories.valid_to || field_histories.valid_to is null)").as_json(include: :field_histories) }
      else
